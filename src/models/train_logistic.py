"""Train a logistic regression baseline with a time-based split."""
...
class TrainingSummary:
    """Summary of logistic regression training artifacts."""
...
def find_newest_parquet(dataset_dir: Path = DEFAULT_DATASET_DIR) -> Path:
    """Find the newest model dataset Parquet file by modification time."""
...
def artifact_stem(input_path: Path) -> str:
    ...
    if "_model_dataset_" in stem:
        ...
        return f"{symbol}_logistic_regression_no_raw_prices_{date_part}"
    return f"{stem}_logistic_regression_no_raw_prices"
...
def train_logistic_regression(
    ...
    """Fit logistic regression on the chronological training split only."""
...
def train_logistic_file(
    input_path: Path,
    model_dir: Path = DEFAULT_MODEL_DIR,
    predictions_dir: Path = DEFAULT_PREDICTIONS_DIR,
    metrics_dir: Path = DEFAULT_METRICS_DIR,
) -> TrainingSummary:
    """Train logistic regression and save model, predictions, and metrics."""
    ...
    model = train_logistic_regression(dataset, feature_columns, splits)
...
def main() -> None:
    args = parse_args()
    input_path = args.input_path or find_newest_parquet(args.dataset_dir)
    summary = train_logistic_file(
